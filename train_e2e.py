...
### modules
from modules.model import CNN, Hybrid, Hybrid_e2e_stn
...
def main(args):
    ...
    data_path = './data/brixia/'
    
    # checkpoint path. (for getting spatiali normalization network from checkpoint/ )
    
    
    
    
    
    model_name = 'brixia_{}_{}_stn_ep{}'.format(args.arch, args.backbone, args.epochs)
    ...
    for epoch in range(EPOCHS):
        ...
        if valid_acc > best_valid_acc:
            ...
            torch.save(model.state_dict(), os.path.join(base_path, 'checkpoints', f'{model_name}_best_acc.pth'))
        
        if valid_loss < best_valid_loss:
            best_valid_loss = valid_loss
            torch.save(model.state_dict(), os.path.join(base_path, 'checkpoints', f'{model_name}_best_loss.pth'))
    
    # Final evaluation
    model.load_state_dict(torch.load(os.path.join(base_path, 'checkpoints', f'{model_name}_best_acc.pth')))
    test_loss, test_acc = evaluate(model, EPOCHS, EPOCHS, test_dataloader, classify_criterion, DEVICE)
    consensus_test_loss, consensus_test_acc = evaluate(model, EPOCHS, EPOCHS, consensus_test_dataloader, classify_criterion, DEVICE)
    
    print(f"Test ACC : {test_acc:.2f}")
    print(f"Consensus Test ACC : {consensus_test_acc:.2f}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight_decay', default=1e-4, type=float, help='weight decay for SGD optimizer')
    ...
    parser.add_argument('--arch', default='hybrid_e2e_stn', type=str, help='[hybrid_e2e_stn]')
    parser.add_argument('--backbone', default='resnet50', type=str, help='[ resnet50 ]')
